...
class GmailToolKit:
    ...
    def mark_email_as_read(self, message_id):
        ...
        if self.service:
            ...
            self.logger.debug(
                "Gmail service not initialized; cannot mark email as read."
            )
    ...
    def get_email_content_based_on_gmail_id(self, message_id):
        ...
        if self.service:
            ...
            self.logger.debug("Gmail service not initialized; cannot fetch email.")
            return None
    ...
    def check_emails(
        ...
        if self.service:
            try:
                ...
                return [
                    self.get_email_content_based_on_gmail_id(m["id"])
                    for m in messages
                    if m
                ]
            ...
            self.logger.debug("Gmail service not initialized; cannot fetch emails.")
    ...
    def send_mail(self, to, subject, body) -> Dict[str, Any]:
        ...
        if self.service:
            ...
            status["success"] = False
            status["message"] = "Gmail service not initialized; cannot send email."
            self.logger.debug("Gmail service not initialized; cannot send email.")
            return status
    ...
    def delete_email(self, message_id: str):
        ...
        if self.service:
            ...
            self.logger.debug("Gmail service not initialized; cannot delete email.")
            status["success"] = False
            status["message"] = f"Gmail service not initialized; cannot delete email."
            return status
...
if __name__ == "__main__":
    ...
    tool = GmailToolKit(gmail_account=gmail_account, run_as_thread=False, max_results=5)
    emails = tool.start()
