...
class TestIntegrationWorkflows:
    ...
    def test_complete_user_onboarding_workflow(
        ...
        oauth_start_response = client.get(
            "/v1/oauth/gmail/start", params=oauth_params, follow_redirects=False
        )
        ...
        account_response = client.post(
            "/v1/test/get_google_account/", json=account_request
        )
        ...
        with patch(
            "app.services.session.session_utils.init_or_get_session"
        ) as mock_init_session:
            with patch("app.utils._api_helper.call_graph") as mock_call_graph:
                mock_call_graph.return_value = (
                    f"Hello {username}, I can help you with email management!"
                )

                with client.websocket_connect(
                    f"/v1/chatbot/open/{username}/{thread_id}"
                ) as websocket:
                    ...
                    assert any(
                        ...
                        for word in ["welcome", "assist", "help", "hello"]
                    )
    ...
    def test_oauth_error_recovery_workflow(
        ...
        oauth_params = {
            "username": username,
            "mobile_session_id": "error_recovery_session",
        }
        oauth_start_response = client.get(
            "/v1/oauth/gmail/start", params=oauth_params, follow_redirects=False
        )
        ...
        with patch("app.api.v1.endpoints.gmail_oauth.oauth_states") as mock_states:
            ...
            callback_response = client.get(
                "/v1/oauth/gmail/callback", params=callback_params
            )
        ...
        oauth_params2 = {
            "username": username,
            "mobile_session_id": "error_recovery_session_2",
        }
        oauth_retry_response = client.get(
            "/v1/oauth/gmail/start", params=oauth_params2, follow_redirects=False
        )
    ...
    def test_concurrent_user_sessions_workflow(
        ...
        with patch(
            ...
            with patch("app.utils._api_helper.call_graph") as mock_call_graph:
                for i, user_data in enumerate(users_data):
                    ...
                    mock_call_graph.return_value = (
                        f"Response for {username}: I can help with your emails"
                    )
                    ...
                    with client.websocket_connect(
                        ...
                        assert any(
                            word in response.lower()
                            for word in ["assist", "help", "can", "how", "understood"]
                        )
        ...
        for i, user_data in enumerate(users_data):
            ...
            thread_id = f"concurrent_thread_{i}"
            with patch(
                "app.api.v1.endpoints.websocket.get_session"
            ) as mock_get_session:
                with patch(
                    "app.api.v1.endpoints.websocket.close_websocket_session"
                ) as mock_close:
    ...
    def test_api_error_handling_workflow(
        self, client: TestClient, mock_database_operations: Dict[str, Any], mock_redis
    ):
        ...
        )  # Returns 200 but with success: false
        assert (
            register_response.json()["success"] is False
        )  # Database error indicated by success: false
        ...
        assert login_response.status_code == 200
        assert (
            login_response.json()["success"] is False
        )  # Authentication error indicated by success: false

        # 3. Test get Google account with error
        # Reset the mock to raise exception instead of returning a value
        mock_database_operations["get_gmail"].side_effect = Exception(
            "Service unavailable"
        )
        mock_database_operations["get_gmail"].return_value = (
            None  # Clear any return_value
        )
    ...
    def test_performance_under_load(
        ...
        def make_request(endpoint_data):
            ...
            return {
                ...
                in [200, 307, 422],  # Expected status codes
            }
