...
def mock_oauth_flow():
    ...
    with patch("app.api.v1.endpoints.gmail_oauth.Flow") as mock_flow, patch(
        "oauthlib.oauth2.is_secure_transport", return_value=True
    ):
        mock_flow_instance = Mock()
        mock_flow_instance.authorization_url = Mock(
            return_value=("http://example.com/oauth", "test_state")
        )
...
def mock_database_operations():
    ...
    with patch("app.api.v1.endpoints.auth.ru") as mock_register, patch(
        "app.api.v1.endpoints.auth.lu"
    ) as mock_login, patch("app.api.v1.endpoints.auth.uud") as mock_update, patch(
        "app.api.v1.endpoints.gmail_oauth.add_gmail_account"
    ) as mock_add_gmail, patch(
        "app.api.v1.endpoints.gmail_oauth.uud"
    ) as mock_oauth_update, patch(
        "app.db.repos.gmail.accounts.get_gmail_account"
    ) as mock_get_gmail, patch(
        "app.api.v1.endpoints.test.get_gmail_account"
    ) as mock_get_gmail_test:
        ...
        mock_oauth_update.return_value = {
            "success": 200,
            "message": "User details updated successfully",
            "updated_data": {},
        }
        mock_add_gmail.return_value = {
            "success": True,
            "message": "Gmail account added successfully",
        }
        mock_get_gmail.return_value = [
            {"email": "test@gmail.com"}
        ]  # Returns list of GmailAccount objects for DB function
        mock_get_gmail_test.return_value = [
            {"email": "test@gmail.com"}
        ]  # Returns list for test endpoint
...
def mock_session_operations():
    ...
    with patch(
        ...
        "app.services.session.get_session.get_session"
    ) as mock_get, patch(
        "app.services.session.delete_session.delete_session"
    ) as mock_delete:
